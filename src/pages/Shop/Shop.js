...
function Shop() {
    ...
    const getFilteredProducts = () => {
        ...
        filtered = filtered.filter(product =>
            product.product_price >= priceRange.min &&
            product.product_price <= priceRange.max
        );
                            ...
                            <>
                                <div className={cx('shop-header')}>
                                    <div className="flex justify-between align-center mb-10px mt-12px">
                                        <div className={cx('counter-product')}>
                                            Showing {indexOfFirstProduct + 1} - {Math.min(indexOfLastProduct, getFilteredProducts().length)} of {getFilteredProducts().length} products
                                        </div>
                                ...
                                <div className="row">
                                    {currentProducts.map((product) => (
                                        <div key={product._id} className="col l-4 m-4 c-6">
