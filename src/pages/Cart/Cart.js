...
function Cart() {
    ...
    const handleIncrease = (index, item) => {
        ...
        const sku = productSkus.find(s =>
            s.sku_color === item.color &&
            s.sku_size === item.size
        );
    ...
    useEffect(() => {
        const fetchSkuData = async () => {
            try {
                const skuPromises = cartItems.map(item =>
                    getDetailProduct(item.slug)
                );
                                ...
                                <td data-title="Discount">
                                    {originalTotal > total ? (
                                        <>
                                            ...
                                            <span className={cx('saved-amount')}>
                                                Save $ {parseFloat(originalTotal - total).toFixed(2)}
                                            </span>
                                        </>
                                    ) : (
                                        'No discount applied'
